...
class GoogleBatchJobProvider(google_utils.GoogleJobProviderBase):
  ...
  def _create_batch_request(
    ...
    compute_resource = build_compute_resource(
      cpu_milli=job_resources.min_cores * 1000,
      memory_mib=job_resources.min_ram * 1024,
      boot_disk_mib=job_resources.boot_disk_size * 1024
    )

    # Bring together the task definition(s) and build the Job request.
    task_spec = google_batch_operations.build_task_spec(
      runnables=runnables, volumes=[datadisk_volume], compute_resource=compute_resource
    )
