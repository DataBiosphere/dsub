...
if [[ "${CHECK_RESULTS_ONLY:-0}" -eq 0 ]]; then
  ...
  if ! gsutil ls "${NEWFILE}"; then
    echo "Job 6 didn't create the file it should have."
    echo "Expected: ${NEWFILE}"
    exit 1
  fi
  ...
  if ! gsutil ls "${OUT_FOLDER_2}/job_7"; then
    echo "Job 7 didn't create the file it should have."
    echo "Expected: ${NEWFILE}"
    exit 1
  fi
  ...
  echo "Checking results only."
fi

PRESENT=$(gsutil ls "${OUTPUTS}" | sort)
EXPECTED=$(echo -e "${EXISTING}\n${NEWFILE}\n${OUT_FOLDER_2}/")
if [[ "${PRESENT}" != "${EXPECTED}" ]]; then
  echo "Unexpected output folder contents."
  echo "Expected:"
  echo "${EXPECTED}"
  echo "Present:"
  echo "${PRESENT}"
